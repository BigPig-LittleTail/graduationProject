    ...
    public List<MyRDF> getRelation(){
        ...
        for(DpNode root:rootNodes){
            if(root.getWord().getPos().equals("v")){
                ...
                List<DpNode> thisSBV = new ArrayList<>();
                List<DpNode> thisVOB = new ArrayList<>();
                for(DpNode childern:childrenNodes){
                    if(childern.getWord().getPos().matches("n[s|h|i|l|z]*") && childern.getWord().getDp().equals("SBV")){
                        thisSBV.add(childern);
                    }
                    else if(childern.getWord().getPos().matches("n[s|h|i|l|z]*") && childern.getWord().getDp().equals("VOB")){
                        thisVOB.add(childern);
                    }
                }
                if(!thisSBV.isEmpty() && !thisVOB.isEmpty()){
                    sbvNodes = thisSBV;
                    vobNodes = thisVOB;
                }
                else if(thisSBV.isEmpty() && thisVOB.isEmpty()){
                    continue;
                }
                else if(thisSBV.isEmpty()){
                    if(sbvNodes.isEmpty()){
                        vobNodes.clear();
                        continue;
                    }
